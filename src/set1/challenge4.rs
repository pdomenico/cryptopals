...
pub fn try_decrypt() {
    ...
    for bytes in lines
        .iter()
        .map(|line| challenge1::hex_decode(line.as_str()).unwrap())
    {
        let freq_table = challenge3::byte_freq_list(&bytes);
        for (byte, _) in freq_table {
            let decrypted = challenge3::single_byte_xor(&bytes, byte ^ ('e' as u8));
            let text = bytes_to_text(&decrypted);
            let score = challenge3::english_score(&text);
